...
interface UserJobListEditProps {
  profileName: string;
  jobs: Job[];
  onEdit: () => void;
  widthClass?: string;
...
function UserJobListEdit({ profileName, jobs, onEdit, widthClass = "w-80", onJobRemoved }: UserJobListEditProps) {
      ...
      <div className="flex flex-col space-y-3">
        {jobs.length === 0 ? (
          ...
          [...jobs]
            ...
            .map((job, index) => <JobCardMini key={index} job={job} edit={true} onJobRemoved={onJobRemoved} />)
        )}
      </div>
    </div>
  );
...
export default UserJobListEdit;
